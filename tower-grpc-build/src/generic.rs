...
macro_rules! {}_impl {{
    ...
    fn blanket_implementations(&self, service: &prost_build::Service, scope: &mut codegen::Scope) {
            ...
            imp.new_fn("call")
                ...
                .push_block({
                    ...
                    for method in &service.methods {
                        ...
                        let match_line = format!("{}::{}(request) =>", &req_name, &upper_name);

                        let mut blk = codegen::Block::new(&match_line);
                        blk.line(&format!(
                            "let fut = self.{}.{}(request);",
                            &lower_name, &method.name
                        ))
        ...
        // impl<T: Service<FooRequest>> Foo for T
        {
            ...
            impgen
                ...
                .bound(
                    &format!("<T as tower_service::Service<{}>>::Future", req_name),
                    "Send + 'static",
                )
                .bound(
                    &format!("<T as tower_service::Service<{}>>::Error", req_name),
            ...
            impgen
                ...
                .line(&format!(
                    "tower_service::Service::poll_ready(self).map_err({}::new)",
                    err_name
                ));
            ...
            for method in &service.methods {
                ...
                let request_type = if method.client_streaming {
                    format!(
                        "Box<futures::Stream<Item={}, Error={}> + Send + 'static>",
                        input_name, err_name
                    )
                } else {
                ...
                let response_type = if method.server_streaming {
                    format!(
                        "Box<futures::Stream<Item={}, Error={}> + Send + 'static>",
                        output_name, err_name
                    )
                } else {
                    output_name.to_string()
                };
                impgen.associate_type(
                    &format!("{}Future", upper_name),
                    &format!(
                        "Box<futures::Future<Item={}, Error={}> + Send>",
                        response_type, err_name
                    ),
                );
    ...
    fn define_wrapper_struct(&self, service: &prost_build::Service, scope: &mut codegen::Scope) {
    ...
    fn impl_future_response(&self, service: &prost_build::Service, scope: &mut codegen::Scope) {
        let svc_name = &service.name;
        ...
        scope
            ...
            .push_block({
                ...
                for method in &service.methods {
                    ...
                    let match_line = format!("{}::{}(ref mut fut) =>", &fut_name, &upper_name);

                    let mut blk = codegen::Block::new(&match_line);
                    blk.line("let response = fut.poll()?;")
    ...
    fn define_request_types(
        ...
        for method in &service.methods {
            ...
            let fut_type = format!(
                "Box<futures::Future<Item={}, Error={}> + Send + 'static>",
                response_type, err_name
            );
        ...
        scope
            ...
            .associate_type(
                &format!("Error = {};\n\nconst PATH: &'static str", err_name),
                path,
            );
